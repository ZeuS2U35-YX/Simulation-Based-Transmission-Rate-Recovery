# ============================================================
# Fit the constant-B model to one shared Experiment 4 data set
#
# Usage:
...
if (length(args) < 1L) {
  stop("Usage: Rscript code/03_run_constant_task.R <task_id> <shared_data_root> <output_root> <paramlist_file>")
}
...
output_root <- if (length(args) >= 3L) args[[3]] else file.path("results_raw", "constant")
...
seed_row <- read_paramlist_task(
  ...
  task_id,
  c(
    "task_id", "simulation_seed", "constant_mif_seed_base",
    "constant_evaluation_seed_base", "constant_final_pf_seed"
  )
)

simulation_seed <- as.integer(seed_row$simulation_seed[[1]])
mif_seed_base <- as.integer(seed_row$constant_mif_seed_base[[1]])
evaluation_seed_base <- as.integer(seed_row$constant_evaluation_seed_base[[1]])
final_pf_seed <- as.integer(seed_row$constant_final_pf_seed[[1]])
...
constant_model <- make_constant_model(observed_data, config)
theta_constant <- constant_baseline_parameters(config)
start_values <- config$constant_start_values
mif_rw_sd <- rw_sd(Beta = config$constant_rw_sd_Beta)
...
for (s in seq_len(nrow(start_values))) {
  ...
  theta_start[["Beta"]] <- start_values$Beta[[s]]

  cat(
    "Running constant-B start ", s, "/", nrow(start_values),
    ": Beta=", theta_start[["Beta"]], "\n", sep = ""
  )
  ...
  if (inherits(mif_now, "mif2_error")) {
    mif_rows[[s]] <- data.frame(
      run = s,
      start_B0 = NA_real_, start_sigma_beta = NA_real_,
      start_Beta = theta_start[["Beta"]],
      B0_hat = NA_real_, sigma_beta_hat = NA_real_, Beta_hat = NA_real_,
      logLik = NA_real_, logLik_se = NA_real_,
      n_successful_pf_evals = 0L,
      status = "mif2_failed", error_message = mif_now$message,
  ...
  if (save_traces) {
    trace_rows[[s]] <- safe_trace_data(
      mif_now, task_id, "constant_B", s,
      list(Beta = theta_start[["Beta"]])
    )
  }

  mif_coef <- tryCatch(coef(mif_now), error = function(e) NULL)
  valid_coef <- !is.null(mif_coef) && is.finite(mif_coef[["Beta"]]) && mif_coef[["Beta"]] > 0
  ...
  if (!valid_coef) {
    mif_rows[[s]] <- data.frame(
      ...
      logLik = NA_real_, logLik_se = NA_real_,
      n_successful_pf_evals = 0L,
      status = "invalid_mif2_estimate", error_message = NA_character_,
      stringsAsFactors = FALSE
    )
    eval_rows[[s]] <- data.frame(
      run = s, evaluation_rep = seq_len(config$n_pf_evals),
  ...
  theta_eval <- theta_constant
  theta_eval[["Beta"]] <- unname(mif_coef[["Beta"]])
  ...
  for (j in seq_len(config$n_pf_evals)) {
    ...
    pf_eval <- tryCatch(
      pfilter(constant_model, params = theta_eval, Np = config$Np_eval),
  ...
  if (length(finite_eval) == 0L) {
    mif_rows[[s]] <- data.frame(
      ...
      start_B0 = NA_real_, start_sigma_beta = NA_real_,
      start_Beta = theta_start[["Beta"]],
      B0_hat = NA_real_, sigma_beta_hat = NA_real_, Beta_hat = theta_eval[["Beta"]],
      logLik = NA_real_, logLik_se = NA_real_,
      n_successful_pf_evals = 0L,
      status = "all_evaluation_pfilters_failed", error_message = NA_character_,
      stringsAsFactors = FALSE
    )
    next
  }
  ...
  cat(
    "Finished start ", s,
    ": Beta_hat=", round(theta_eval[["Beta"]], 5),
...
if (nrow(valid) == 0L) {
  ...
  theta_best <- theta_constant
  theta_best[["Beta"]] <- best$Beta_hat[[1]]

  set.seed(final_pf_seed)
  pf_best <- tryCatch(
    pfilter(constant_model, params = theta_best, Np = config$Np_final),
    error = function(e) structure(
  ...
  best_summary <- data.frame(
    best_run = best$run[[1]],
    start_B0 = NA_real_, start_sigma_beta = NA_real_,
    start_Beta = best$start_Beta[[1]],
    B0_hat = NA_real_, sigma_beta_hat = NA_real_,
    Beta_hat = best$Beta_hat[[1]],
...
write_run_config(
  ...
  list(
    ...
    n_pf_evals = config$n_pf_evals,
    Np_final = config$Np_final,
    starts_Beta = paste(start_values$Beta, collapse = ","),
    rw_sd_Beta = config$constant_rw_sd_Beta,
...
cat("Constant-B task status: ", best_summary$status[[1]], "\nSaved to ", atomic$final_dir, "\n", sep = "")
