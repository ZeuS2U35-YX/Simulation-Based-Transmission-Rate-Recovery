# ============================================================
# Experiment 1C: fit the Gamma-noise model to piecewise-B simulated data
...
#
# Because both are log-transformed in pf_gamma_model,
...
for (i in seq_len(nrow(start_values))) {
  ...
  # Because this is inside the i-loop, the seed is reset
  # before every mif2 run.
  ...
  # Replace only B0 and sigma_beta with the mif2 estimates.
  ...
  for (j in seq_len(n_pf_evals)) {
    ...
    #
    # For example, all nine results use:
...
# and use the selected mif2 estimates for B0 and sigma_beta
...
write.csv(
  mif_results,
  file.path(results_directory, "piecewise_estimated_B0_mif2_results.csv"),
  row.names = FALSE
)

write.csv(
  best_fit,
  file.path(results_directory, "piecewise_estimated_B0_best_fit.csv"),
  row.names = FALSE
)

saveRDS(
  mif_best,
  file.path(results_directory, "piecewise_estimated_B0_best_mif2.rds")
)

pdf(
  file = file.path(figures_directory, "piecewise_estimated_B0_mif2_diagnostic.pdf"),
  width = 7.2,
  height = 4.6
)
...
write.csv(
  B_estimate,
  file.path(results_directory, "piecewise_estimated_B0_filtered_B_path.csv"),
  row.names = FALSE
)

write.csv(
  infectious_estimate,
  file.path(results_directory, "piecewise_estimated_B0_filtered_infectious_path.csv"),
  row.names = FALSE
)
...
B_path_plot <- ggplot() +
  ...
  geom_vline(xintercept = 5, linetype = "dashed", linewidth = 0.4, color = "gray70") +
  scale_color_manual(
    values = c(
      "True B(t)" = "black",
...
ggsave(
  filename = file.path(figures_directory, "piecewise_estimated_B0_filtered_B_path.pdf"),
  plot = B_path_plot,
  width = 6.2,
  height = 4.2,
  units = "in",
...
ggsave(
  filename = file.path(figures_directory, "piecewise_estimated_B0_filtered_infectious_path.pdf"),
...
ggsave(
  filename = file.path(figures_directory, "piecewise_estimated_B0_starting_value_loglik.pdf"),
